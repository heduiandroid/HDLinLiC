...
public class GoodsBySearchedAdapter extends Adapter<ViewHolder> {
    private Context context;
    private List<DirectGoodBean.DataBean> list;

    public GoodsBySearchedAdapter(Context context, List<DirectGoodBean.DataBean> list) {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.vertical_goods_ver_cell,null);
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        if (holder instanceof  GoodViewHolder){
            ...
            if (salecate != null){
                ((GoodViewHolder) holder).tv_notice.setTextColor(context.getResources().getColor(R.color.lightred));
                if (salecate == 1L){
                ...
                ((GoodViewHolder) holder).tv_notice.setTextColor(context.getResources().getColor(R.color.notselect_text));
                ((GoodViewHolder) holder).tv_notice.setText("品牌："+list.get(position).getBrand());
            }
            ((GoodViewHolder) holder).tv_soldout.setText("已售" + soldNum + "件");
            ((GoodViewHolder)holder).container.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    UIHelper.togoGoodsDetailActivity(context,list.get(position).getId(),"");
                }
            });
        }
    }
    ...
    private class GoodViewHolder extends ViewHolder{
        ...
        TextView tv_good_name,tv_notice,tv_price,tv_soldout;
